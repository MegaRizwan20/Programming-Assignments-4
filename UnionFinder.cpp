 ...
 * Date: 6/5/17
 *
 * This is for finding the sets and joining them together
 */
...
{
  // Destruct everyting in the movie vectors
  for (int i = 0; i < allMovies.size(); i++)
  {
    delete allMovies[i];
  }

  // Delete all the iterators that we need to
...
bool UnionFinder::loadFromFile(const char* in_filename)
{
  ...
  while (infile) 
  {
    ...
    // Get the name of the actor, movie name, and year of the movie
...
void UnionFinder::printStats( ostream& out ) const
{
  ...
  // since each movieGraph creates a complete graph, 
  // which has a total number of edges of n*(n-1), we just need to find 
  // the number of edges generated in each movieGraph
  // and sum them all up
  int sum = 0;
  for (auto it = movieList.listOfGraphs.begin();
          it != movieList.listOfGraphs.end(); it++)
  {
...
// print all the actor names and the earliest year in which 
// they are connected to the output stream
void UnionFinder::printAllYears( std::vector< std::pair< 
                            std::string, std::string> > pairs, ostream& out )
{
  ...
  for (auto it = allNodes.begin(); it != allNodes.end(); it++)
  {
    ...
    while (it_year != allYears.end() )
    {
      ...
      while (it_graph != movieList.listOfGraphs.end() &&
                                  (*it_graph)->getMovieYear() == *it_year )
      {
        curr = *it_graph;
        ...
        {
          if (curr->listOfActors[i]->prev == nullptr && 
                           curr->listOfActors[i]->getFirstEdge() == nullptr)
          {
            sentinel = i;
            break;
          }
        ...
        if (sentinel != -1)
        {
          for (int i = 0; i < curr->listOfActors.size(); i++)
          {
            // check if both the parent pointer and 
                          ...
                          curr->listOfActors[i]->getFirstEdge() == nullptr)
            {
              if ( curr->listOfActors[i] != curr->listOfActors[sentinel] )
              {
                ...
                curr->listOfActors[sentinel]->addEdge(curr->listOfActors[i], 
                                                      curr->getMoviePointer());
              }
             }
             ...
             else
             {
               if (i != sentinel)
               {
                 curr->listOfActors[sentinel]->unionWith(curr->listOfActors[i],
                                                      curr->getMoviePointer());
               }
             }
           }
         ...
         else
         {
           for (int i = 0; i < curr->listOfActors.size(); i++)
           {
             ...
             {
               curr->listOfActors[i]->unionWith(curr->listOfActors[j], 
                                                   curr->getMoviePointer());
             }
           }
         }
      ...
      // used to check if every single pairs are found already. 
      // If yes, break out of main loop
      bool allFound = true;
    ...
    // print the actor names and found years to the output stream 
    // that is given as a parameter
    for (int i = 0; i < allFoundYears.size(); i++)
    {
      // no path is found if the found year for the pair is 
      // still 0 at this point
      if (allFoundYears[i] == 0) allFoundYears[i] = 9999;
      out << pairs[i].first << "\t" << pairs[i].second << "\t" << allFoundYears[i] << "\n";
    }
}

// print all the actor names and the earliest year in which they 
// are connected to the output stream
// This function uses the 'existPath' function, which is basically 
// using bfs to check if there exists a path between a pair of nodes
void UnionFinder::bfsAllPairs( std::vector< std::pair< std::string,
                               std::string> > pairs, ostream& out )
{
  // initialize a vector of found years
  std::vector<int> allFoundYears( pairs.size(), 0 );
  std::vector< std::pair< ActorNode *, ActorNode *> > pointerPairs;
  ...
  while (it_year != allYears.end() )
  {
    ...
    // used to check if every single pairs are found already. If yes,
...
// Checks the existence of a path if there is one between nodes
